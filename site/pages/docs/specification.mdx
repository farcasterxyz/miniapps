...
are an OpenGraph-inspired metadata standard that let's any page in an Mini App
rendered as a rich object that can launch user into an application.
...
`/.well-known/farcaster.json`. The Manifest includes all the data needed for a
