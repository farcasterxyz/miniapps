...
- **Non-atomic execution**: The Farcaster Wallet executes transactions sequentially, not atomically. If one transaction fails, subsequent transactions may not be executed.
- **No paymaster support**: Gas fees must be paid by the user's wallet for each transaction.
- **Transaction scanning**: All transactions in the batch are validated and scanned for security, maintaining the same safety guarantees as individual transactions.
:::
