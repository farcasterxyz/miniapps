...
The Farcaster Wallet now supports EIP-5792 `wallet_sendCalls`, allowing you to batch multiple transactions into a single user confirmation. This improves user experience by enabling operations like "approve and swap" in one step.
...
With Wagmi, send multiple transactions as a batch using the `sendCalls` method:
