...
### Category for search and browse

Set **`primaryCategory`** in your manifest to one of the [documented category slugs](/docs/specification#manifest) (for example `games` or `art-creativity`). Discovery surfaces can index this field as a **category facet** so users can filter or search by type. Use the exact slug from the schema (lowercase, hyphenated), not a display name.
