...
Because this provider works with `@solana/web3.js` transaction objects,
`@solana/web3.js` is an optional peer dependency. To use the low-level provider,
install `@solana/web3.js` and opt in by importing `@farcaster/miniapp-sdk/solana`
...
```tsx
// Enables sdk.wallet.getSolanaProvider(). Import once in your app entry.
...
Mini apps that use `@farcaster/mini-app-solana` (the recommended Wallet Standard
integration above) do not need this — that path does not use the low-level
provider.
